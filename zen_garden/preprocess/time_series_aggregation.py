            ...
            for year in self.optimization_setup.year_specific_ts:
                ...
                for element,ts in elements_time_series: #zip(elements,time_series):
                    ...
                    if (element,ts) in self.optimization_setup.year_specific_ts[year].keys():
                        ii = unstacked[element, ts].index #TODO to avoid indexing past lexsort depth PerformanceWarning
                        unstacked[element,ts] = self.optimization_setup.year_specific_ts[year][(element,ts)][ii]  #TODO to avoid indexing past lexsort depth PerformanceWarning
                    else:
